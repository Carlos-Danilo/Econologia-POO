...
class Morador:
    ...
    @staticmethod
    def from_json(dic):
        return Morador(dic["id"], dic["nome"], dic["email"], dic["fone"],dic["senha"])
    def __str__(self):
...
class MoradorDAO(DAO):
    ...
    def abrir(cls):
        cls._objetos = []
        try:
            with open("Morador.json", mode="r") as arquivo:
                list_dic = json.load(arquivo)
                for dic in list_dic:
                    obj = Morador.from_json(dic)
    ...
    @classmethod
    def salvar(cls):
        with open("Morador.json", mode="w") as arquivo:
            json.dump(cls._objetos, arquivo, default = Morador.to_json) 